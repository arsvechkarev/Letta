...
/**
 * An interaction behavior plugin for a child view of {@link CoordinatorLayout} to make it
 * work as a bottom sheet.
 *
 * <p>To send useful accessibility events, set a title on bottom sheets that are windows
 * or are BottomSheetDialogFragment use {@link ViewCompat#setAccessibilityPaneTitle(View,
 * CharSequence)}.
 */
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
   ...
   * destroyed and recreated. The only value preserved will be the positional state, e.g.
   * collapsed, hidden, expanded, etc. This is the default behavior.
   */
  ...
  public BottomSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    TypedArray a = context
        .obtainStyledAttributes(attrs, R.styleable.BottomSheetBehavior_Layout);
    this.shapeThemingEnabled = a
        .hasValue(R.styleable.BottomSheetBehavior_Layout_shapeAppearance);
    createShapeValueAnimator();

    if (VERSION.SDK_INT >= VERSION_CODES.LOLLIPOP) {
      this.elevation = a
          .getDimension(R.styleable.BottomSheetBehavior_Layout_android_elevation, -1);
    }

    TypedValue value = a
        .peekValue(R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight);
    ...
    setHalfExpandedRatio(
        a.getFloat(R.styleable.BottomSheetBehavior_Layout_behavior_halfExpandedRatio,
            0.5f));
    setExpandedOffset(
        a.getInt(R.styleable.BottomSheetBehavior_Layout_behavior_expandedOffset, 0));
    a.recycle();
  ...
  public boolean onLayoutChild(
    ...
    if (ViewCompat.getFitsSystemWindows(parent) && !ViewCompat
  ...
  public boolean onTouchEvent(
    ...
    if (action == MotionEvent.ACTION_MOVE && !ignoreEvents) {
      if (Math.abs(initialY - event.getY()) > viewDragHelper.getTouchSlop()) {
        viewDragHelper
            .captureChildView(child, event.getPointerId(event.getActionIndex()));
      }
    }
    return !ignoreEvents;
  }
    ...
    if (lastNestedScrollDy > 0) {
      ...
      if (fitToContents) {
        ...
        if (currentTop < halfExpandedOffset) {
          ...
          if (Math.abs(currentTop - halfExpandedOffset) < Math
              .abs(currentTop - collapsedOffset)) {
            top = halfExpandedOffset;
            targetState = STATE_HALF_EXPANDED;
          } else {
  ...
  /**
   * @return whether the height of the expanded sheet is determined by the height of its
   * contents, or if it is expanded in two stages (half the height of the parent
   * container, full height of parent container).
   */
  ...
  /**
   * Sets whether the height of the expanded sheet is determined by the height of its
   * contents, or if it is expanded in two stages (half the height of the parent
   ...
   * #PEEK_HEIGHT_AUTO} to configure the sheet to peek automatically at 16:9 ratio
   * keyline.
  ...
  /**
   * Sets the height of the bottom sheet when it is collapsed while optionally animating
   * between the old height and the new height.
   *
   * @param peekHeight The height of the collapsed bottom sheet in pixels, or {@link
   * #PEEK_HEIGHT_AUTO} to configure the sheet to peek automatically at 16:9 ratio
   * keyline.
   ...
   * #PEEK_HEIGHT_AUTO} if the sheet is configured to peek automatically at 16:9 ratio
   * keyline
   * @attr ref com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_peekHeight
   */
  ...
  /**
   * Determines the height of the BottomSheet in the {@link #STATE_HALF_EXPANDED} state.
   * The material guidelines recommended a value of 0.5, which results in the sheet
   * filling half of the parent. The height of the BottomSheet will be smaller as this
   * ratio is decreased and taller as it is increased. The default value is 0.5.
   *
   * @param ratio a float between 0 and 1, representing the {@link #STATE_HALF_EXPANDED}
   * ratio.
  ...
  /**
   * Determines the top offset of the BottomSheet in the {@link #STATE_EXPANDED} state
   * when fitsToContent is false. The default value is 0, which results in the sheet
   * matching the parent's top.
   *
   * @param offset an integer value greater than equal to 0, representing the {@link
   * #STATE_EXPANDED} offset. Value must not exceed the offset in the half expanded
   * state.
  ...
  /**
   * Gets the ratio for the height of the BottomSheet in the {@link #STATE_HALF_EXPANDED}
   * state.
   *
   * @attr com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_halfExpandedRatio
   */
  ...
  /**
   * Sets whether this bottom sheet should skip the collapsed state when it is being
   * hidden after it is expanded once. Setting this to true has no effect unless the sheet
   * is hideable.
   *
  ...
  /**
   * Sets whether this bottom sheet should skip the collapsed state when it is being
   * hidden after it is expanded once.
   *
   ...
   * {@link #SAVE_HIDEABLE}, {@link #SAVE_SKIP_COLLAPSED}, {@link #SAVE_ALL} and {@link
   * #SAVE_NONE}.
  ...
  /**
   * Sets the state of the bottom sheet. The bottom sheet will transition to that state
   * with animation.
   *
   * @param state One of {@link #STATE_COLLAPSED}, {@link #STATE_EXPANDED}, {@link
   * #STATE_HIDDEN}, or {@link #STATE_HALF_EXPANDED}.
   */
   ...
   * #STATE_COLLAPSED}, {@link #STATE_DRAGGING}, {@link #STATE_SETTLING}, or {@link
   * #STATE_HALF_EXPANDED}.
   */
  @State
  public int getState() {
  ...
  void setStateInternal(@State int state) {
    ...
    } else if (state == STATE_HALF_EXPANDED || state == STATE_HIDDEN
  ...
  private void restoreOptionalState(@NonNull SavedState ss) {
    ...
    if (this.saveFlags == SAVE_ALL
        || (this.saveFlags & SAVE_PEEK_HEIGHT) == SAVE_PEEK_HEIGHT) {
  ...
  void startSettlingAnimation(View child, int state, int top,
      boolean settleFromViewDragHelper) {
  ...
  protected static class SavedState extends AbsSavedState {
    ...
    /**
     * This constructor does not respect flags: {@link BottomSheetBehavior#SAVE_PEEK_HEIGHT},
     * {@link BottomSheetBehavior#SAVE_FIT_TO_CONTENTS}, {@link
     * BottomSheetBehavior#SAVE_HIDEABLE}, {@link BottomSheetBehavior#SAVE_SKIP_COLLAPSED}.
     * It is as if {@link BottomSheetBehavior#SAVE_NONE} were set.
     *
  ...
  /**
   * A utility function to get the {@link BottomSheetBehavior} associated with the {@code
   * view}.
   *
   ...
   *
   * <p>Set this to true if the expanded state of the sheet blocks access to siblings
   * (e.g., when the sheet expands over the full screen).
   */
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (expanded) {
        ...
        if (updateImportantForAccessibilityOnSiblings
          ...
          ViewCompat.setImportantForAccessibility(child,
              importantForAccessibilityMap.get(child));
        }
      }
    }
